package res;

import java.awt.*;

public class Styles {

    public static int PAGE_TITLE_FONTSIZE = 60;

    public static Font UNICODE_FONT = new Font("Code2000", Font.PLAIN, 25);

    public static Color PAGE_TITLE_FOREGROUNGCOLOR = new Color(0x5D8EFF);
    public static Color SEARCHING_COLOR = Color.red;
    public static Color SWAP_COLOR = PAGE_TITLE_FOREGROUNGCOLOR ;
    public static Color FOUND_COLOR = Color.green;
    public static Color ELEMENT_COLOR = Color.white;
    public static Color DATA_COLOR = Color.red;
    public static Color APP_BACKGROUNDCOLOR = new Color(0x343434);
}
